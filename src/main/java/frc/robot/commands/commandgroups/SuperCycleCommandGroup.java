...
public class SuperCycleCommandGroup extends SequentialCommandGroup {
    ...
    public SuperCycleCommandGroup(ReefTargeting target, SwerveSubsystem s, ElevatorSubsystem e, CoralizerSubsystem c, ShooterSubsystem sh, CameraSubsystem ca, DistanceSubsystem d) {
        ...
        ReefTargeting dealgPose = getDealgTarget(target.getTarget().getPosition(), s);
        ...
        addCommands(
                ...
                Commands.deadline(
                        ...
                        new ElevatorPositionCommand(e, dealgPose.getTarget().getLevel().height, dealgPose.getTarget().getLevel())
                ),
                Commands.parallel(
                        new FastAlignReefCommand(dealgPose.getAlignmentOffsetPose().get(), 0.75, s),
                        new ElevatorPositionCommand(e, dealgPose.getTarget().getLevel().height, dealgPose.getTarget().getLevel())
                ),
                Commands.parallel(
