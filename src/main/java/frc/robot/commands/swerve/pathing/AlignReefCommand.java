...
public class AlignReefCommand extends SequentialCommandGroup {
    ...
    public AlignReefCommand(Pose2d reefSidePose, Pose2d reefPose, Supplier<Pose2d> currentPose, PathConstraints fastConstraints, SwerveSubsystem s) {
        ...
        Pose2d offsetReefPose = MathHelper.offsetPoseReverse(reefSidePose, 0.6 + (cRobotLength / 2));

        addCommands(
                new PathFindCommand(offsetReefPose, fastConstraints, s),
                new PathToTwoPosesCommand(offsetReefPose, MathHelper.offsetPoseReverse(reefPose, (cRobotLength / 2) - 0.02), 0, slowConstraints, s),
                new AccuratePathCommand(MathHelper.offsetPoseReverse(reefPose, (cRobotLength / 2) - 0.02), 1.5, true, s)
        );
